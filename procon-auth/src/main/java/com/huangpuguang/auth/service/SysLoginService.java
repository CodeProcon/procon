package com.huangpuguang.auth.service;
...
public class SysLoginService
{
    ...
    {
        // 用户名或密码为空 错误
        if (ProconStrUtils.isAnyBlank(username, password))
        {
        ...
        // 查询用户信息
        ResultModel<LoginUser> userResult = remoteUserService.getUserInfo(username, SecurityConstants.INNER);

        if (ResultModel.FAILED == userResult.getCode())
        {
            throw new ServiceException(userResult.getMsg());
        }

        if (ProconStrUtils.isNull(userResult.getData()))
        {
    ...
    public void register(String username, String password)
    {
        ...
        ResultModel<?> registerResult = remoteUserService.registerUserInfo(sysUser, SecurityConstants.INNER);

        if (ResultModel.FAILED == registerResult.getCode())
        {
    ...
    public void recordLogininfor(String username, String status, String message)
    {
        ...
        // 日志状态
        if (ProconStrUtils.equalsAny(status, Constants.LOGIN_SUCCESS, Constants.LOGOUT, Constants.REGISTER))
        {
